...
#   - MariaDB database server (Alma/Rocky native for Express v1)
...
install_dynportal_firewall() {
    ...
    # Certbot's SSL vhost uses /var/www/html as DocumentRoot.
    # Add aliases so dynportal works on both HTTP and HTTPS:
    #   https://FQDN/valid8.php
    #   https://FQDN/dynportal/valid8.php
    cat > /etc/httpd/conf.d/01-genx-dynportal-alias.conf <<'EOF_DYN_ALIAS'
Alias /valid8.php /var/www/vhosts/dynportal/valid8.php
Alias /dynportal /var/www/vhosts/dynportal

<Directory "/var/www/vhosts/dynportal">
    DirectoryIndex valid8.php index.html
    AllowOverride None
    Require all granted
</Directory>

<Directory "/var/www/vhosts/dynportal/inc">
    Require all denied
</Directory>
EOF_DYN_ALIAS
...
start_and_verify_vicidial() {
    echo
    echo "=================================================="
    echo " Starting and verifying VICIdial processes"
    echo "=================================================="
...
SQL_FINAL_SERVER
    ...
    local screen_count
    screen_count="$(screen -ls 2>/dev/null | grep -c 'Detached' || true)"
    log "Detected detached screen count: ${screen_count}"

    if [[ "${screen_count}" -lt 10 ]]; then
        log "WARNING: Expected at least 10 VICIdial/Asterisk screens on Express install."
        log "Run: /usr/share/astguiclient/ADMIN_keepalive_ALL.pl --cu3way --debugX"
    fi
